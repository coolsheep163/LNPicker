...
from mmdet.apis import multi_gpu_test, single_gpu_test, init_random_seed, set_random_seed
...
def main():
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(model, data_loader, args.show, args.show_dir,
                                  args.show_score_thr)
        num_gts = dataset.custom_count_gts()
        num_dets = 0
        for out in outputs:
            num_dets += sum([_out.shape[0] for _out in out])
        print(f'\nNumber of ground truth boxes: {num_gts}, '
              f'\nNumber of detections: {num_dets}')
    else:
        model = build_ddp(
            model,
            cfg.device,
